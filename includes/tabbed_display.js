/**
 *  @fileoverview The class Tabbed_Display is the display handler for direct tabbed displays.
 *
 *	@version 0.6.1  [17.7.2008]		// start of prototype 6
 *	@version 0.6.2  [18.7.2008]
 *	@version 0.6.3  [6.10.2008]
 */
...
/**
 *	Constructor for the Tabbed_Display class.
 *	@class	The class Tabbed_Display provides support for the display/re-display
 *	operations for fixed-tab tab-structured displays that switch between constant
 *	content. So no internal operations are needed to add or re-configure displays
 *	nor to re-process content before display. This class requires the existence
...
function Tabbed_Display(a_sDisplay_Name)
{
	/**
	...
	/**
	 *	The name of the display section containing the tabs.
	 *	@type	string
	 */
	this.m_sSelection_Section = '';
	...
	/**
	 *	The unique name of the current active/selected tab.
	 *	Extracted from the link 'href' attribute not the tab label.
	 *	@type	string
	 */
	this.m_sActive_Tab = '';

	/**
	 *	The list that maps each tab name to its HTML section ID.
	 *	An associative array indexed by the unique tab names.
	 *	@type	array
	 */
	this.m_asDisplay_List = null;
...
Tabbed_Display.prototype.set_element_names = function(a_sPanel_Element, a_asElement_List)
{
	var nx = 0;
	...
	// The tab section elements are listed last and must be listed in the same order 
	// as the tabs (regardless of the order the sections are declared on the page).
	// Currently there are 2 non-tab elements expected, so they will be listed first.
	var nInitial_Element_List_Length = 2;

	// Expect at least one tab.
	if (!is_array(a_asElement_List)
		|| (a_asElement_List.length < (nInitial_Element_List_Length + 1))) {
		alert("Bad element list for display '" + this.m_sDisplay_Name + "'.");
		return (false);
	}

	var nElement_List_Length = a_asElement_List.length;
	...
	this.m_sSelection_Section = a_asElement_List[0];
	this.m_sList_Element = a_asElement_List[1];

	// Now we're only handling the tabs.
	this.m_sActive_Tab = '';
	this.m_asDisplay_List = Array();		// Do I need to set up the array here to ensure that
											// each tabbed_display object has its own copy ???

	var eTab_List = document.getElementById(this.m_sList_Element);
	var nTotal_Tab_Count = nElement_List_Length - nInitial_Element_List_Length;

	var aeTab_Links = eTab_List.getElementsByTagName('a');
	var nTab_Link_Count = aeTab_Links.length;
	if (nTab_Link_Count != nTotal_Tab_Count) {
		alert("Tab mismatch found for the '" + this.m_sDisplay_Name + "' element.");
		return (false);
	}

	var sTab_Name = '';
	for (nx=0; nx<nTotal_Tab_Count; nx++) {
		sTab_Name = aeTab_Links[nx].getAttribute("href").split('#')[1];
		this.m_asDisplay_List[sTab_Name] = a_asElement_List[nInitial_Element_List_Length + nx];
		if (aeTab_Links[nx].className == 'selected') {
			if (this.m_sActive_Tab.length > 0) {
				aeTab_Links[nx].className = '';
			}
			else {
				this.m_sActive_Tab = sTab_Name;
			}
		}
	}

	// Finally attach the event handler to all tabs.
	//eTab_List = document.getElementById(this.m_sList_Element);
	var aeTab_Elements = eTab_List.getElementsByTagName('li');
	for (nx=0; nx<nTotal_Tab_Count; nx++) {
		// Under DOM2 can have multiple event handlers registered.
		remove_event(aeTab_Elements[nx], 'click', tab_link_click, false);
		add_event(aeTab_Elements[nx], 'click', tab_link_click, false);
	}
...
Tabbed_Display.prototype.hide_display = function()
{
	...
	var sActive_Section_Name =  this.m_asDisplay_List[this.m_sActive_Tab];
	eDisplay_Element = document.getElementById(sActive_Section_Name);
	if (!eDisplay_Element) {
		alert("Display section element '" + sActive_Section_Name + "' no longer exists.");
		return;
	}
	eDisplay_Element.className = 'hidden';

	eDisplay_Element = document.getElementById(this.m_sSelection_Section);
	if (!eDisplay_Element) {
		alert("Display section element '" + this.m_sSelection_Section + "' no longer exists.");
		return;
	}
	eDisplay_Element.className = 'hidden';
}
...
Tabbed_Display.prototype.show_display = function()
{
	var eDisplay_Element = document.getElementById(this.m_sSelection_Section);
	if (!eDisplay_Element) {
		alert("Display section element '" + this.m_sSelection_Section + "' no longer exists.");
		return;
	}
	...
	eDisplay_Element = document.getElementById(this.m_sPanel_Element);
	if (!eDisplay_Element) {
		alert("Display panel element '" + this.m_sPanel_Element + "' no longer exists.");
		return;
	}
...
Tabbed_Display.prototype.tab_activation = function(a_eTarget_Tab)
{
	...
	this.select_new_section(sTarget_Name);		// Because this operation can fail,
												// call it before the tab is selected
...
/**
 *	Switch to a new display section.
 *
 *	@param	{string} a_sSection_Name One of the unique configured display section names.
 *
 *  @return	None.
 *  @type	void
 */

Tabbed_Display.prototype.select_new_section = function(a_sSection_Name)
{
	var sElement_Name = '';
	var eDisplay_Element = null;

	if (is_null(a_sSection_Name) || (!is_string(a_sSection_Name))) {
		return;
	}

	if ((is_undefined(this.m_asDisplay_List[a_sSection_Name]))
		|| (is_null(this.m_asDisplay_List[a_sSection_Name]))) {
		alert('Unknown display section \'' + a_sSection_Name + '\' selected.');
		return;
	}

	if (this.m_sActive_Tab != a_sSection_Name) {
		if (this.m_sActive_Tab != '') {
			sElement_Name = this.m_asDisplay_List[this.m_sActive_Tab];
			eDisplay_Element = document.getElementById(sElement_Name);
			if (!eDisplay_Element) {
				alert("Display section element '" + sElement_Name + "' no longer exists.");
				return;
			}
			this.m_sActive_Tab = '';
			eDisplay_Element.className = 'hidden';
		...
		if (a_sSection_Name != '') {
			sElement_Name = this.m_asDisplay_List[a_sSection_Name];
			eDisplay_Element = document.getElementById(sElement_Name);
			if (!eDisplay_Element) {
				alert("Display section element '" + sElement_Name + "' no longer exists.");
				return;
		...
		this.m_sActive_Tab = a_sSection_Name;
	}
}
...
Datatable_Controller.prototype.request_fieldset_download = function()
{
	...
	rMessage_Transfer_Service.request_transfer('table', this.m_sDisplay_Name, FIELDSET_PAGE_URL, 
...
Tabbed_Display.prototype.print_state = function()
{
